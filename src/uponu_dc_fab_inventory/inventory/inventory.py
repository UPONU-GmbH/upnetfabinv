...
class Inventory(InventoryFacts):
    ...
    def all(self) -> dict:
        ...
        for leaf in self.shared_utils.l3leafs:
            inventory_tree["NETWORK_SERVICES"]["children"][get(leaf, "name", required=True)] = dict()
    ...
    def _get_fabric(self, fab: str) -> dict:
        ...
        children = {
            f"{fab}_SPINES": {
                "hosts": self._get_spines(fab)
            },
            f"{fab}_L3_LEAVES": {
                "hosts": self._get_l3leafs(fab)
            },
            f"{fab}_L2_LEAVES": {
                "hosts": self._get_l2leafs(fab)
            }
        }
