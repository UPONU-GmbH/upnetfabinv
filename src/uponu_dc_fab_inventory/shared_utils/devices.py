...
class DevicesMixin:
    ...
    def _device_interfaces_and_ip(self: SharedUtils, device: dict) -> list[dict]:
        interfaces = self.netbox.dcim_interfaces_filter(
            device_id=get(device, "id", required=True)
        )
        ip_addresses = self.netbox.ipam_ip_addresses_filter(
            device_id=get(device, "id", required=True)
        )
            ...
            res.append(
                {
                    ...
                    "ips": get_all(
                        get_all_items(
                            ip_addresses,
                            "assigned_object_id",
                            get(interface, "id", required=True),
                        ),
                        "address",
                    ),
                    "interface": interface,
